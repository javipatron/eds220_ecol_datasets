...
library(jsonlite)
library(here)
library(rnaturalearth) #Easy accesible map polygons

```


###IUCN API (https://apiv3.iucnredlist.org/api/v3)
Red List

Assemble a URL to access the data from the API
...
#print a string and use the %s to subsitute with the 
...
# Humpback Whale: 13006 (IUCN species ID number)
...
```{r}
am_spp_cells_df <- read_csv("data/am_mammal_hcaf_cropped_v10_2019.csv")
```
...
longlat_df <- am_hcaf_df |> 
  filter(ocean_area >0) |> 
  select(center_long, center_lat, loiczid)


hcaf_r <- rast(longlat_df, type = "xyz", crs = "epsg: 4326")

plot(hcaf_r)
```


## Start Joining specific columns for that species
```{r}
...
#Create a massive join with the both data sets
```{r}
...
## Analysis 1: map orca suitavility
...
plot(orca_prob_r, main = "Probability of Ocurrance for Orcinus Orca")
...
## Analysis 3: average extition risk
...
mean_risk_df <- risk_cells_df |> 
  filter(prob >= 0.5) |> 
  group_by(loiczid) |> 
  summarize(mean_risk = mean(cat_score, na.rm = T))
...
mean_risk_r[mean_risk_r > 5] <-NA
  plot(mean_risk_r, main = "Mean extition risk")
